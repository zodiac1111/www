...
function fdex(type, noOfRow) {
	...
	if (3 != type && 5 != type && 6 != type) {
		document.paraform.indexno.value = noOfRow;
		// 传行号
	}
	return true;
}
...
function GoForNPage(ward) {
	...
	var PageCount = 1;
	if (ward == -1) {
		document.paraform.pageselect.value = curpagevalue - (curpagevalue == 0 ? 0 : 1);
	...
	if (null != document.paraform.pageindex.value && document.paraform.pageindex.value != "") {
		fdex(3, -1);
		// 准备提交换页命令
...
function all_ie_changed(e) {
	...
	curobj.style.color = "green";
	// 数值改变了,字体变绿色
	if (rNums.test(curobj.value) && curobj.value >= 0 && curobj.value < 10.0) {// 数值错误:
		curobj.style.backgroundColor = "";
	} else {// 正确则恢复
...
function ie_changed(e) {
	...
	curobj.style.color = "green";
	// 数值改变了,字体变绿色
	if (rNums.test(curobj.value) && curobj.value >= 0 && curobj.value < 10.0) {
		curobj.style.backgroundColor = "";
		curobj.id = "";
	} else {
		curobj.style.backgroundColor = "red";
